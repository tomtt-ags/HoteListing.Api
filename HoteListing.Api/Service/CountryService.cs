...
public class CountryService(HoteListingDbContext context, IMapper mapper) : ICountryService
{
    ...
    public async Task<Result<GetCountryDto>> GetAsync(int id)
    {
     ...
     public async Task<Result> UpdateCountryAsync(int id, UpdateCountryDto updateDto)
    {
        try
        {
            if (id != updateDto.Id)
            {
                return Result.Failure(new Error("Validation", "Id route value does not match payload Id."));
            }
